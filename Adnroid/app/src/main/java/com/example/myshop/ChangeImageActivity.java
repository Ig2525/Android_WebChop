...
public class ChangeImageActivity extends AppCompatActivity {
    ...
    public void onClickCrop(View view) {
        String fileTemp = java.util.UUID.randomUUID().toString();   //Робимо рандомне імя файла
        Bitmap croppedBitmap = cropView.getOutput();                //
        Matrix matrix = new Matrix();                               // Записуємо поворот
        ...
        Uri fileSavePath = Uri.fromFile(new File(getCacheDir(), fileTemp)); // Записуємо файл в кеш
        CropUtil.saveOutput(this, fileSavePath, rotatedBitmap, 90);// Записуєм файл з поворотом

        //Вертаємо результат - Шлях до файлу, який збережено через кропер
        Intent intent = new Intent();
        intent.putExtra("croppedUri", fileSavePath); // Вказуємо шлях де зберегли результюче фото
